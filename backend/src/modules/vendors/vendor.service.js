...
const getAllVendors = async (tenantId, options = {}) => {
  ...
  const [rows, countResult] = await Promise.all([
    query(
      `${baseSql} ORDER BY ${orderBy} ${order} LIMIT ? OFFSET ?`,
      [...params, limit, offset]
    ),
...
/**
 * Soft delete: set is_active = 0. Vendors table has is_active.
 */
const deleteVendor = async (id, tenantId) => {
  const pool = getPool();
  const [result] = await pool.execute(
    'UPDATE vendors SET is_active = 0 WHERE id = ? AND tenant_id = ?',
    [id, tenantId]
  );
